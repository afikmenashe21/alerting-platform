...
# ECS Task Definition
# Using bridge network mode for better efficiency on small instances (no ENI limits)
resource "aws_ecs_task_definition" "service" {
  family                   = "${var.project_name}-${var.environment}-${var.service_name}"
  network_mode             = "bridge"  # Changed from awsvpc to avoid ENI limits on t3.micro
  ...
  container_definitions = jsonencode([
    {
      ...
      # For bridge mode, use dynamic host port mapping (0 = auto-assign)
      portMappings = var.container_port > 0 ? [
        {
          containerPort = var.container_port
          hostPort      = 0  # Dynamic port for bridge mode
          protocol      = "tcp"
        }
      ] : []
...
# ECS Service
# Note: No network_configuration needed for bridge network mode
resource "aws_ecs_service" "service" {
  ...
  # network_configuration not needed for bridge mode - removed to fix deployment
